...
import br.com.dio.picpayclone.data.BandeiraCartao
...
class TransacaoFragment : Fragment() {
    ...
    private fun observarErro() {
        transacaoViewModel.onError.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            it?.let { mensagem ->
    ...
    private fun observarTransferencia() {
        transacaoViewModel.transacao.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            val direcao = TransacaoFragmentDirections.actionNavigationTransferenciaToNavigationPagar()
